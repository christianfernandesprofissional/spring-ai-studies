...
//Usando RAG mas com dados sendo carregados diretamente no código
//@Component //comentar a annotation caso não quiser esta implementação
public class RandomDataLoader {

    // O VectorStore é uma abstração que representa e acessa uma coleção vetorial persistida externamente,
    //delegando toda a responsabilidade de armazenamento ao banco vetorial subjacente.
    ...
    @PostConstruct //Anotação para dizer ao spring que assim que o Construtor for executado, o Spring deve executar este metodo logo em seguida
    public void loadSentencesIntoVectorStore(){
        //Lista de frases para dar um contexto a IA, onde ela só vai responder usando as informações contidas neste documento,
        //do contrário a IA irá responder com "I don't know"
        //Estas frases serão armazenadas no VectorStore e transformadas em números relacionados a contexto
        ...
        List<Document> documents = sentences.stream().map(Document::new).collect(Collectors.toList()); //transformando as frases em documentos
